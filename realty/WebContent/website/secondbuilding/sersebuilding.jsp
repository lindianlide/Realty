...
List<Secondhouse> buildingList=building.secondList();
System.out.println("paa");
int pageSize=10;
int index=0;
int count=building.secondList().size();
...
List<Secondhouse> pagelist=building.secondList(pageNo, pageSize);
...
function passpageNo(){ 
	...
	document.form.action="website/secondbuilding/sersebuilding.jsp?pageNo="+gopageNo;
...
<div id="content" class="content">
					...
					<table width="742">
	      			   <%for(int i=0;i<pagelist.size();i++){ 
	      				 List BuildingPhotoList=building.secondPhoto(pagelist.get(i).getHouseId(),"sedecorate_photo");
	      				String aString =null;
	      				String bString =null;
	      				...
	      				case 4: bString="混合";break;
	      				 ...
	      				 if(!BuildingPhotoList.isEmpty()){%>
		      			   <tr height="150">
		      			   	<td  width="150"><img style="width:150px;height:170px;" alt="" src="picupload/second/<%=BuildingPhotoList.get(0)%>"></td>
		      			   	<%}else{%>
		      			   	<tr height="150">
		      					<td  width="150">无图</td>
		      				 <% }%>
		      			    <td><div id="tdtitle"><a href="website/secondbuilding/sebuildingintro.jsp?houseId=<%=pagelist.get(i).getHouseId()%>"> <%=pagelist.get(i).getHouseName()%></a></div>
		      			    <div id ="tdtxt1">地址：<%=pagelist.get(i).getPropertyAdress() %></div>
		      			    <div id ="tdtxt2">户型：<%=pagelist.get(i).getHouseType()%></div>
		      			    <div id ="tdtxt2">装修情况：<%=aString %></div>
		      			    <div id ="tdtxt3">建筑类型：<%=bString %></div>
		      			     <div id ="tdtxt4">电话：</div></td>
		      			    <td><div><%=pagelist.get(i).getExchangePrice() %></div></td>
		      			   </tr>
		      			   <%} %>
		      			    </table>
        ...
        <tr>
          ...
          <table style="border:0px;"  width="100%" class="right-font08">
              <tr>
                ...
                <td style="border:0px;" width="48%" align="right">[<a href="website/secondbuilding/sersebuilding.jsp?pageNo=1" class="right-font08">首页</a> | <a href="website/secondbuilding/sersebuilding.jsp?pageNo=<%=pageNo-1%>" class="right-font08">上一页</a> | <a href="website/secondbuilding/sersebuilding.jsp?pageNo=<%=pageNo+1%>" class="right-font08">下一页</a> | <a href="website/secondbuilding/sersebuilding.jsp?pageNo=<%=index%>" class="right-font08">末页</a>] 转至：</td>
