...
<%@page import="com.realty.base.model.Micro"%>
...
<%
 ...
 %>
 <%
 String miccategoryId1=request.getParameter("gory");
 if(miccategoryId1==null)
	 miccategoryId1="1";
 int miccategoryId=Integer.parseInt(miccategoryId1);
 WebsiteAction website=new WebsiteAction();
 int pageSize=2;
 int index=0;
 int count=website.newsList(miccategoryId).size();
 ...
 List<Micro> pagelist=website.microList(pageNo, pageSize,miccategoryId);
 %>
	...
	<div id="panelfo-in2" >
	  <ul class="lmenu">
		<li style="margin-bottom:20px">
			<a href="micro.jsp?earth=aa&&gory=0" style="font-size:20px;color:#000;background:#EBEBEB">市场动态</a>
			<!-- <ul class="level2">
		...
		<li style="margin-bottom:20px">
			<a href="micro.jsp?earth=aa&&gory=1" style="font-size:20px;color:#000;background:#EBEBEB">政策信息</a>
			<!-- <ul class="level2">
		...
		<li style="margin-bottom:20px">
			<a href="micro.jsp?earth=aa&&gory=2" style="font-size:20px;color:#000;background:#EBEBEB">土地咨询</a>
		<!-- 	<ul class="level2">
		...
		<li style="margin-bottom:20px">
			<a href="micro.jsp?earth=aa&&gory=3" style="font-size:20px;color:#000;background:#EBEBEB">公司动态</a>
			<!-- <ul class="level2">
	 ...
	 <% 
		 ...
		 {%>
             ...
             <div id="Layer6">
  				<div id="Layer7"><a href="micro.jsp?earth=bb&&microid=<%=pagelist.get(i).getMicroId()%>" ><%=pagelist.get(i).getName()%></a></div>
  				<div id="Layer8"><%=pagelist.get(i).getData()%></div>
  					<div id="Layer9">
    				<div id="Layer10"><a href="micro.jsp?earth=bb&&microid=<%=pagelist.get(i).getMicroId()%>" >详情</a></div>
    				</div>
  			</div>
        ...
        <tr>
          <td height="33"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="right-font08">
              <tr>
                ...
                <td width="48%" align="right">[<a href="../dynpages/news.jsp?earth=aa&gory=<%=miccategoryId%>&pageNo=1" class="right-font08">首页</a> | <a href="../dynpages/news.jsp?earth=aa&gory=<%=miccategoryId %>&pageNo=<%=pageNo-1%>" class="right-font08">上一页</a> | <a href="../dynpages/news.jsp?earth=aa&gory=<%=miccategoryId %>&pageNo=<%=pageNo+1%>" class="right-font08">下一页</a> | <a href="../dynpages/news.jsp?earth=aa&gory=<%=miccategoryId %>&pageNo=<%=index%>" class="right-font08">末页</a>] </td>
		 ...
		 <%}
		 if("bb".equals(names)){
		  	int mircocategoryId =Integer.parseInt(request.getParameter("microid"));
		     WebsiteAction weba = new WebsiteAction();
		     List<Micro> list2=weba.microList(mircocategoryId);
		     %>
			 <div>
				   <div style="text-align:center;color:#000000"> <%=list2.get(0).getName()%></div>
                  <div style="float:right;color:#000000;margin:10px 10px">时间： <%=list2.get(0).getData()%></div>
