...
if(culcategoryId==0)
	title="企业文化列表";
/* else if(culcategoryId==1)
	title="政策信息列表";
else if(culcategoryId==2)
	title="土地咨询列表";
else if(culcategoryId==3)
	title="公司动态列表"; */
...
int pageSize=2;
int index=0;
int count=website.cultureList(culcategoryId).size();
...
List<Culture> pagelist=website.cultureList(pageNo, pageSize,culcategoryId);
...
function insert()
{
	window.open('manage/web/culture/culture_add.jsp?culcategoryId=<%=culcategoryId%>','middleFrame','top=0,left=0,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
}
function search()
{
	document.form.action="WebsitemServlet?flag=16";
	document.form.submit();
}
function passpageNo(){
	var gopageNo = document.getElementById("gopageNo").value; 
	document.form.action="manage/web/culture/culture_list.jsp?culcategoryId=<%=culcategoryId%>&pageNo="+gopageNo;
	document.form.submit();
}
</SCRIPT>
          ...
          <td><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
              ...
              <tr>
                <td class="font42"><table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#464646" class="newfont03">
                  ...
                  <tr align="center">
				    <td height="30" bgcolor="#FFFFFF"> <a href="manage/web/culture/culture_detail.jsp?culcategoryId=<%=culcategoryId%>&culture_id=<%=pagelist.get(i).getCultureId()%>" ><%=pagelist.get(i).getTitle()%></a></td>
                    <td bgcolor="#FFFFFF"><a href="manage/web/culture/culture_update.jsp?culcategoryId=<%=culcategoryId%>&culture_id=<%=pagelist.get(i).getCultureId()%>" >修改 </a>   <a href="WebsitemServlet?flag=15&culcategoryId=<%=culcategoryId%>&cultureId=<%=pagelist.get(i).getCultureId()%>">删除</a></td>
                  </tr>
                  <%} %>
                </table></td>
              </tr>
        ...
        <tr>
          <td height="33"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="right-font08">
              <tr>
                ...
                <td width="48%" align="right">[<a href="manage/web/culture/culture_list.jsp?culcategoryId=<%=culcategoryId %>&pageNo=1" class="right-font08">首页</a> | <a href="manage/web/culture/culture_list.jsp?culcategoryId=<%=culcategoryId %>&pageNo=<%=pageNo-1%>" class="right-font08">上一页</a> | <a href="manage/web/culture/culture_list.jsp?culcategoryId=<%=culcategoryId %>&pageNo=<%=pageNo+1%>" class="right-font08">下一页</a> | <a href="manage/web/culture/culture_list.jsp?culcategoryId=<%=culcategoryId %>&pageNo=<%=index%>" class="right-font08">末页</a>] 转至：</td>
