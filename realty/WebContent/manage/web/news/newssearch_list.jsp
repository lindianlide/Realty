...
List<News> searchlist=(List<News>)session.getAttribute("newssearchlist");
%>
<% 
...
function insert()
{
	window.open('manage/web/news/news_add.jsp','middleFrame','top=0,left=0,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
}
function search()
{
          ...
          <td><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
              ...
              <tr>
                <td class="font42"><table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#464646" class="newfont03">
                  ...
                  <tr align="center">
				    <td height="30" bgcolor="#FFFFFF"> <a href="manage/web/news/news_detail.jsp?news_id=<%=searchlist.get(i).getNewsId()%>&newscategoryId=<%=searchlist.get(i).getNewscategoryId()%>" ><%=searchlist.get(i).getTitle()%></a></td>
                    <td bgcolor="#FFFFFF"><a href="manage/web/news/news_update.jsp?news_id=<%=searchlist.get(i).getNewsId()%>&newscategoryId=<%=searchlist.get(i).getNewscategoryId()%>" >修改 </a>   <a href="WebsitemServlet?flag=7&newsId=<%=searchlist.get(i).getNewsId()%>&newscategoryId=<%=searchlist.get(i).getNewscategoryId()%>" >删除</a></td>
                  </tr>
                  <%} %>
                </table></td>
              </tr>
