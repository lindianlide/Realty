...
if(macrocategoryId==0)
	title="国际列表";
else if(macrocategoryId==1)
	title="国家列表";
else if(macrocategoryId==2)
	title="山东列表";
...
int pageSize=2;
int index=0;
int count=website.macroList(macrocategoryId).size();
...
List<Macro> pagelist=website.macroList(pageNo, pageSize,macrocategoryId);
%>
<% 
...
function insert()
{
	window.open('manage/web/macro/macro_add.jsp?macrocategoryId=<%=macrocategoryId%>','middleFrame','top=0,left=0,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
}
function search()
{
	document.form.action="WebsitemServlet?flag=24";
	document.form.submit();
}
function passpageNo(){
	var gopageNo = document.getElementById("gopageNo").value; 
	document.form.action="manage/web/macro/macro_list.jsp?macrocategoryId=<%=macrocategoryId%>&pageNo="+gopageNo;
	document.form.submit();
}
</SCRIPT>
          ...
          <td><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
              ...
              <tr>
                <td class="font42"><table width="100%" border="0" cellpadding="4" cellspacing="1" bgcolor="#464646" class="newfont03">
                  ...
                  <tr align="center">
				    <td height="30" bgcolor="#FFFFFF"> <a href="manage/web/macro/macro_detail.jsp?macrocategoryId=<%=macrocategoryId%>&macro_id=<%=pagelist.get(i).getMacroId()%>" ><%=pagelist.get(i).getName()%></a></td>
                    <td bgcolor="#FFFFFF"><a href="manage/web/macro/macro_update.jsp?macrocategoryId=<%=macrocategoryId%>&macro_id=<%=pagelist.get(i).getMacroId()%>" >修改 </a>   <a href="WebsitemServlet?flag=23&macrocategoryId=<%=macrocategoryId%>&macroId=<%=pagelist.get(i).getMacroId()%>">删除</a></td>
                  </tr>
                  <%} %>
                </table></td>
              </tr>
        ...
        <tr>
          <td height="33"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="right-font08">
              <tr>
                ...
                <td width="48%" align="right">[<a href="manage/web/macro/macro_list.jsp?macrocategoryId=<%=macrocategoryId %>&pageNo=1" class="right-font08">首页</a> | <a href="manage/web/macro/macro_list.jsp?macrocategoryId=<%=macrocategoryId %>&pageNo=<%=pageNo-1%>" class="right-font08">上一页</a> | <a href="manage/web/macro/macro_list.jsp?macrocategoryId=<%=macrocategoryId %>&pageNo=<%=pageNo+1%>" class="right-font08">下一页</a> | <a href="manage/web/macro/macro_list.jsp?macrocategoryId=<%=macrocategoryId %>&pageNo=<%=index%>" class="right-font08">末页</a>] 转至：</td>
